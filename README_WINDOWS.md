...
仓库有更新时，**双击 `update.bat`** 就会：

1. `git pull --rebase --autostash origin main` 拉最新代码
2. `uv sync` 同步依赖（如果 `pyproject.toml` 没变会秒过）
3. 自动重启后台服务，新版本立即生效

`.env` 是被 `.gitignore` 忽略的本地文件，**不会被覆盖**。

> 或者双击 `control.bat → [6] 检查更新`，效果一样。

### 前置条件

- 当前目录是通过 `git clone` 得来的（里面有 `.git` 文件夹）
- 机器上装了 [Git for Windows](https://git-scm.com/download/win)

**如果没装 git**：去 GitHub 网页下 ZIP 覆盖本目录也可以（`.env` 保留即可）。但强烈推荐装 git，以后更新就是一次双击的事。
